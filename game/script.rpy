...
label fifthAct:
    ...
    ggDenis "<Так вот как зовут этого легендарного сетевика>"
    Todd "Что? То есть ты один за один день своим одним глазом выследил человека, которого десятки лет ищет куча фирм с их толпами офисников? {w}Товарищи, меня одного это смущает?"

    menu:
        "..."
        "(Поддержать Тодда) Согласен, не клеится как-то ваша история, капитан.":
            call disputeWithUrgenFirst
        "(Промолчать)":
            call disputeWithUrgenSecond
    ...
    ggDenis "<Странно, зачем Мурусу так подставлять Тодда?>"
    ...
    Murus "Нет смысла. За кольцом бдят офисники."
    ...
    Todd "У меня не очень, а вот у Брайна точно всё получилось. За время {a=glossary:Делёжки}\"Делёжки\"{/a} он поработал почти на все крупные фирмы континента и всех успешно кинул: кого-то обокрал, кого-то сдал другим, некоторых может контролировать. Или мог до недавнего времени."
    ...
    ggDenis "И какое же?"
    Murus "Так как Брайн боится, что тобой всё ещё могут управлять офисники, он попросил выключить тебе имплант зрения."
    ...
    Mira "Сделано."
    ggDenis "<Какие \"тёплые\" воспоминания нахлынули на меня>"
    ...
    ggDenis "<Снова какое-то жжение в затылке>"
    ...
    Urgen "Ещё раз говорю, в этом теле загружен {a=glossary:Образ}образ{/a}, который ты украл месяц назад у содружества \"NOW\", {w}и он испорчен, чем я невероятно огорчён, и очень хочу, чтобы ты починил его."
    Brayn "В смысле починить? Это образ, а не холодильник с пивом. Его просто так не восстановить."
    Brayn "Открою тебе секрет, не всякая фирма способна позволить себе такое, а я вообще один."
    Urgen "Открою тебе секрет, меня это не волнует. {w}Я заплатил немаленькую сумму за этот образ и очень хочу, чтобы он работал корректно."
    ...
    Brayn "Ладно, ладно. {w}Посмотрим, как встал образ. {w}Будет немножко больно."
    ggDenis "<Ай, голова греется изнутри>"
    Brayn "Образ встал отлично. {w}С какой попытки удалось?"
    Urgen "С седьмой."
    ...
    Brayn "Пока что стандартно. А ещё что-нибудь? Может быть, не работали какие-нибудь импланты?"
    ggDenis "Да, я ничего не видел, после наш {a=glossary:Сетевик}сетевик{/a} помог мне."
    Brayn "А вот это странно, в каждом образе есть инструкции для использования имплантов. {w}Они загружаются в новое тело вместе с основным блоком информации."
    ...
    stop music
    #((Белый экран с ультразвуком из первого акта))
    scene bg_interference with dissolve
    play sound ультразвук
    ...
    Brayn "Влад, плохие новости. Его {a=glossary:Образ}образ{/a} буквально перелопачен, стёрто очень много воспоминаний, что-то удалено, как инструкции к имплантам."
    ggDenis "<Какой Влад? К кому он обращается?>"
    ...
    Brayn "И подключили к ней половину мира, чтобы обезопасить себя от нападений из Сети, как во времена {a=glossary:Делёжки}Делёжки{/a}."
    ...
    call firstСhoiceToddLabel

    Murus "Того робота подкинули ребята Рекса. {w}Насколько я понял, их прижали \"NOW\" и предложили выбор. {w}Либо они дохнут вместе с нами, либо становятся пешками {a=glossary:Офисник}офисников{/a} и подкидывают нам вирус."
    ...
    Murus "Тодд, тут пригодился твой опыт со времён {a=glossary:Делёжки}\"Делёжки\"{/a}, когда фирмы постоянно подкидывали друг другу всякие вирусы внутри техники. {w}И я подумал, что ты, как инженер, должен прекрасно знать, как работают вирусы."
...
label disputeWithUrgenFirst:
    ...
    Urgen "Я поставил следить за Брайном одного своего знакомого, ещё когда ты пошёл забирать образ."
...
label firstСhoiceToddLabel:
    ...
    if firstСhoiceTodd:
        ...
        # ((Цвет глаза меняется на красный))
...
default glossary_dict = {
    ...
    "Newborn Old World": "Содружество крупны фирм в Евразии.",
